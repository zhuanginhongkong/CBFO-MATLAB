  classdef CBFO < GUITopo  % A CLASS OF COLORED BODY-FITTED OPTIMIZATION (CBFO)
    properties
      Param
      Input
      Output
    ...
    methods
      function obj = CBFO( nelx, nely, volfrac, loadcase)
        obj.Param.penal          =   3 ;
        obj.Param.rmin           =   6 ;
      ...
      function obj = Optimize(obj)
        ...
        [ p, t, ~, ~, Ve, pmid] = GenMesh( 1, xn, yn, BDY) ;
        ...
        saveas(gcf,'./final.png') ; xphy = ones(length(t), 1); xphy(1:length(t1)) = 1e-9 ;
    ...
    methods (Static)
      ...
      function [ H, Hs] = FilterIndex( t, pmid, rmin)
        ...
        for i1 = 1 : length(t)
          ...
          node_r1 =   reshape(t(i1,:),numel(t(i1,:)),1) ;
          for i = 1 : length(node_r1)
            [tri,~]  = find(t==node_r1(i)) ;
            tri_r1 = [ tri_r1; tri(:)] ;
          end
          tri_r1=unique(tri_r1) ;
          node_r2 = unique( reshape(t(tri_r1,:), numel(t(tri_r1,:)),1) ) ;
          node_r2 = setdiff(node_r2,node_r1) ;
          for i = 1:length(node_r2)
            [tri,~]  = find(t==node_r2(i)) ;
            tri_r2 = [tri_r2;tri(:)] ;
          end
          tri_r2=unique(tri_r2) ;
          node_r3 =  unique(reshape(t(tri_r2,:),numel(t(tri_r2,:)),1) ) ;
          node_r3 = setdiff(node_r3,[node_r2 ; node_r1]) ;
          for i = 1:length(node_r3)
            [tri,~]  = find(t==node_r3(i)) ;
            tri_r3 = [tri_r3;  tri(:)] ;
          end
          tri_rall=unique([tri_r3;tri_r2 ;tri_r1]) ;
          for i2 = 1:length(tri_rall)
            k = k+1 ;
            iH(k) = i1 ;
            jH(k) = tri_rall(i2) ;
            sH(k) = max(0,rmin-sqrt((pmid(i1,1)-pmid(tri_rall(i2),1))^2+(pmid(i1,2)-pmid(tri_rall(i2),2))^2)) ;
          end
        end
        H = sparse( iH, jH, sH) ;
        Hs = sum( H, 2) ;
      ...
      function [ dCe, J] = FiniteElementAnalysis( t, p, BDY, x, E0, Emin, nu, penal, loadcase)
        ...
        for i = 1 : NT
          ...
          dCe(i) = -penal*(E0-Emin)*x(i).^(penal-1).*(Ce(i)./(Emin+x(i).^penal*(E0-Emin)));
        end
      ...
      function [ xnew, change] = OptimalityCriteriaUpdate( x, dCe, Ve, volfrac, t, p, loop, nelx, nely)
        ...
        drawnow ; saveas(gcf,['./fig', int2str(loop) '.png']);
      end
    end
  end
